...
def car_list():
    ...
    with open("Vehicles.txt","r") as vehiclesfile:
        Vehicles=csv.reader(vehiclesfile)
        for row in Vehicles:
            if row[0] in rentedcars:
                continue
...
def rent_car():
    with open("Vehicles.txt","r",encoding='utf-8') as vehiclesfile, open("rentedVehicles.txt","r",encoding='utf-8') as rentedfile:
        Vehicles=csv.reader(vehiclesfile, delimiter=',')
        RentVehicles=csv.reader(rentedfile, delimiter=',')
        rentedcars= []
        avaliable_cars =[]
        ...
        if car_reg not in avaliable_cars:
            print('Car not available')
        elif car_reg in rentedcars:
            ...
            with open("Customers.txt","r",encoding='utf-8') as customersfile:
                ...
                for row in Customers:
                    if b_day == row[0]:
                        ...
                        print('Hello {} \nYou rented the car {}'.format(row[1],car_reg))
                    ...
                    elif b_day not in row[0]:
                        fullname = input('Please Enter your Fulll name (firstname then last name): ')
                        fname = fullname.split(" ")[0].capitalize()
                        lname = fullname.split(" ")[1].capitalize()

                        while True:
                            email = str(input('Pleaes enter your email name: '))
